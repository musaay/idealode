...
func (f *fakeStore) ListIdeasFiltered(ctx context.Context, flt IdeaFilter) ([]Idea, error) {
	f.lastFilter = flt
	if f.err != nil {
		return nil, f.err
	}
...
func (f *fakeStore) GetIdeaBySlug(ctx context.Context, slug string) (*Idea, error) {
	if f.err != nil {
		return nil, f.err
	}
	for i := range f.ideas {
...
func (f *fakeStore) Blend(ctx context.Context, slug string, lang string) (*Idea, error) {
	...
	return &Idea{ID: 77, Slug: "kart-77", Title: "Türetilmiş kart", SourceType: "ai_blended"}, nil
}

func (f *fakeStore) IdeaSources(ctx context.Context, slug string) ([]IdeaSource, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func TestWrappedNotFoundStillRenders404(t *testing.T) {
	fs := sampleStore()
	fs.err = fmt.Errorf("api (/api/ideas/kart-1): %w", ErrNotFound)
...
func (p *panicStore) ListIdeasFiltered(ctx context.Context, f IdeaFilter) ([]Idea, error) {
...
func TestDoubtfulFilterChip(t *testing.T) {
	...
	if fs.lastFilter.Flag != FlagDoubtful {
	...
	if fs2.lastFilter.Flag != FlagDoubtful || fs2.lastFilter.SourceType != "market_derived" {
