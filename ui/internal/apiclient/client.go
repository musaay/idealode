...
//
// Hata sözleşmesi: kayıt yoksa web.ErrNotFound (web 404 render eder),
...
func (c *Client) ListIdeasFiltered(ctx context.Context, f web.IdeaFilter) ([]web.Idea, error) {
	q := url.Values{}
	if f.SourceType != "" {
	...
	// Beyaz liste web.FlagDoubtful'dedir; tanınmayan değer hiç gönderilmez.
	if f.Flag == web.FlagDoubtful {
		q.Set("flag", f.Flag)
	}

	var body struct {
		Ideas []web.Idea `json:"ideas"`
	}
...
// GetIdeaBySlug, `GET /api/ideas/{slug}` — tek kart; yoksa web.ErrNotFound.
// `parent_idea_id`/`parent_slug` ve `mine` alanları web.Idea üzerinde
// çözülür (#110: URL yüzeyi artık slug, sayısal id'ye asla düşülmez).
func (c *Client) GetIdeaBySlug(ctx context.Context, slug string) (*web.Idea, error) {
	var body struct {
		Idea *web.Idea `json:"idea"`
	}
...
func (c *Client) Blend(ctx context.Context, slug string, lang string) (*web.Idea, error) {
	var body struct {
		Idea *web.Idea `json:"idea"`
	}
...
// sourceDTO, IdeaSource'un tel üzerindeki biçimi. web.IdeaSource'un kendi
...
func (c *Client) IdeaSources(ctx context.Context, slug string) ([]web.IdeaSource, error) {
	var body struct {
	...
	out := make([]web.IdeaSource, 0, len(body.Sources))
	for _, s := range body.Sources {
		out = append(out, web.IdeaSource{
			Platform:  s.Platform,
...
//
// Durum eşlemesi: 404 -> web.ErrNotFound; 400 -> web.ErrBadRequest;
...
func statusError(path string, code int) error {
	...
	case http.StatusNotFound:
		return fmt.Errorf("api (%s): %w", path, web.ErrNotFound)
